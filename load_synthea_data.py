...
def validate_inputs(args: argparse.Namespace) -> Tuple[Path, str, str]:
    ...
    if not args.container_url:
        sys.exit("Missing container SAS URL. Set FHIR_IMPORT_CONTAINER_SAS_URL or pass --container-url.")
...
def trigger_import(
    ...
    payload: Dict[str, object] = {
        ...
        "inputSource": container_url,
        "input": inputs,
    }

    print("Starting $import operation...")
    response = requests.post(f"{fhir_url}/$import", headers=headers, json=payload, timeout=30)
    if response.status_code not in {200, 202}:
        sys.exit(f"$import failed: {response.status_code} {response.text}")

    status_url = response.headers.get("Content-Location")
    if not status_url:
        sys.exit("FHIR service did not return a Content-Location header for status polling.")
...
def poll_import_status(
    ...
    while True:
        ...
        if response.status_code >= 400:
            sys.exit(f"$import status failed: {response.status_code} {response.text}")

        retry_after = int(response.headers.get("Retry-After", interval_seconds))
        print(f"Import still running (status {response.status_code}). Waiting {retry_after}s...")
...
def main() -> None:
    ...
    if args.wait:
        poll_import_status(credential, fhir_url, status_url, args.poll_interval)
    else:
